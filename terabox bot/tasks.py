...
# Platform aliases -> canonical name. Used as referrer sources.
...
# TeraBox share link patterns - supports all known domains
TERABOX_DOMAINS = (
    "terabox.com",
    "teraboxapp.com",
    "1024terabox.com",
...
TERABOX_RE = re.compile(
    rf"^https?://(?:www\.)?(?:{'|'.join(re.escape(d) for d in TERABOX_DOMAINS)})/s/\S+$",
    re.IGNORECASE,
)
...
def parse_link(raw: str) -> str:
    ...
    if not TERABOX_RE.match(raw):
        raise ParseError(
            f"'{raw}' is not a TeraBox link — expected one of: {', '.join(TERABOX_DOMAINS)}"
        )
    return raw
...
_QueueItem = tuple[Task, Progress, Callable[[Task, Optional[str], Optional[str]], Awaitable[None]]]
queue: Optional[asyncio.Queue[_QueueItem]] = None
_queue_event: Optional[asyncio.Event] = None


def init_queue() -> None:
    """Must be called from inside the running event loop."""
    global queue, _queue_event
    queue = asyncio.Queue()
    _queue_event = asyncio.Event()


def enqueue_item(item: _QueueItem) -> None:
...
def request_stop(task_id: Optional[int] = None) -> list[Task]:
    """Ask jobs to wind down. `None` means every live job.

    Only sets a flag -- the handler stops at its next checkpoint, so views
