"""Admin-only Telegram bot for generating TeraBox views.
...
def _parse_user_id(command: CommandObject, message: Message) -> Optional[int]:
...
HELP = (
    ...
    "Example:\n"
    "<code>https://teraboxapp.com/s/1abcXYZ telegram 500</code>\n\n"
    "<b>Speed</b>\n"
    "<code>parallel</code> is how many views run at the same time — this is the "
    "speed dial, not <code>delay</code>. Default {lanes}, max {max_lanes}.\n"
    "<code>https://teraboxapp.com/s/1abcXYZ telegram 500 0 default 50</code> — 50 at once\n"
    "Failed views are retried, so <code>count</code> means that many real views.\n\n"
    "<b>Supported TeraBox domains:</b>\n"
    "<code>terabox.com, teraboxapp.com, 1024terabox.com, 4funbox.com, "
    "teraboxlink.com, terasharefile.com, terafileshare.com, terasharelink.com</code>\n\n"
    "<b>Platforms (referrer sources):</b> {platforms}\n\n"
    "<b>Run it daily by itself</b>\n"
    "<code>/schedule &lt;link&gt; &lt;platform&gt; &lt;min&gt;-&lt;max&gt; &lt;start&gt; &lt;end&gt; [HH:MM]</code>\n"
    "Example — a random 500-800 views every night at 1 AM, from today until 10 Sept:\n"
    "<code>/schedule https://teraboxapp.com/s/1abcXYZ telegram 500-800 today 2026-09-10</code>\n"
    "Time defaults to <b>01:00</b> ({tz}). Dates: YYYY-MM-DD, today, tomorrow or +N days.\n\n"
    "<b>Commands</b>\n"
    "/id — show your user id\n"
    "/status — what is running, and the queue\n"
    "/accounts — TeraBox account status & reload\n"
...
async def cmd_stopschedule(message: Message, command: CommandObject) -> None:
    ...
    lines = [
        f"Stopping <b>{len(stopped)}</b> job(s) — views already in flight "
        "will finish, then each posts where it got to."
    ]
    lines += [f"• <code>#{t.id}</code> {t.platform} x{t.count} ({t.status})" for t in stopped]
    await message.answer("\n".join(lines))
...
async def cmd_status(message: Message) -> None:
    ...
    jobs = live_tasks()
    running = "\n".join(
        f"• <code>#{t.id}</code> {t.platform} x{t.count} — {t.status}" for t in jobs
    )
    await message.answer(
        (f"<b>Jobs</b>\n{running}\n\n" if jobs else "Nothing running.\n\n")
        + f"Queued: <b>{_tasks.queue.qsize()}</b>\n"
        f"Workers: <b>{WORKER_COUNT}</b>\n"
        f"Views at once (default): <b>{DEFAULT_CONCURRENCY}</b> (max {MAX_CONCURRENCY})\n"
        f"Daily schedules: <b>{len(active)}</b>\n"
        f"TeraBox accounts: <b>{account_store.count()}</b>"
    )


@router.message(Command("accounts"))
async def cmd_accounts(message: Message) -> None:
    """Show TeraBox account status and reload from disk."""
    account_store.reload()
    count = account_store.count()
    await message.answer(
        f"<b>TeraBox Accounts</b>\n"
        f"Loaded: <b>{count}</b>\n"
        f"Cooldown: <b>{account_store.cooldown}s</b> per account\n\n"
        f"{'Ready to generate views.' if count > 0 else 'No accounts found! Add accounts to accounts.json'}\n\n"
        f"<code>accounts.json</code> format:\n"
...
def describe_schedule(sched: Schedule, now: datetime) -> str:
    ...
    return (
        f"<code>#{sched.id}</code> <b>{sched.platform}</b> x{sched.count_text()} "
        f"daily at {sched.run_time:%H:%M}\n"
        f"{html.escape(sched.link)}\n"
        f"{sched.start_date} → {sched.end_date} · ran {sched.runs}x · next: {when}"
    )
...
async def submit_jobs(message: Message) -> None:
    ...
    if not tasks:
        if not errors:
            await message.answer(
                "Send: <code>&lt;link&gt; &lt;platform&gt; &lt;count&gt;</code>\n"
                "Link must be a TeraBox share URL."
            )
        return
...
async def enqueue(task: Task, bot: Bot, chat_id: int, note: str = "") -> None:
    ...
    header = f"<code>#{task.id}</code> {task.platform} x{task.count}\n{html.escape(task.link)}"
    if note:
        header = f"{note}\n{header}"
    ...
    async def done(finished: Task, result: Optional[str], error: Optional[str]) -> None:
        body = f"{html.escape(result or 'done')}" if error is None else f"Error: {html.escape(error)}"
        with contextlib.suppress(Exception):
            await status_msg.edit_text(f"{header}\n\n{body}")

    track(task)  # so /stopschedule can reach it, queued or running
    q = _tasks.queue
    await q.put((task, progress, done))
...
async def main() -> None:
    ...
    async def fire(sched: Schedule, task: Task) -> None:
        note = f"Daily schedule #{sched.id} — {now_local():%Y-%m-%d %H:%M}"
