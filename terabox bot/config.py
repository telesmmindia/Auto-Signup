...
# roughly 0.5-2 seconds, so 20 lanes is about 20x the views per second.
...
# --- TeraBox accounts for view counting ----------------------------------
# Views only count when the viewer is a logged-in TeraBox user.
# Store accounts as a JSON list: [{"email": "...", "password": "..."}, ...]
ACCOUNTS_FILE = BASE_DIR / "accounts.json"

# Cooldown before reusing an account (seconds). Prevents the same account
# from being used too frequently which could trigger rate limits.
ACCOUNT_COOLDOWN_SECS = int(os.environ.get("ACCOUNT_COOLDOWN_SECS", "300"))  # 5 min


class AccountStore:
    """Pool of TeraBox accounts that rotate for view generation."""

    def __init__(self, path: Path = ACCOUNTS_FILE, cooldown: int = ACCOUNT_COOLDOWN_SECS):
        self.path = path
        self.cooldown = cooldown
        self._accounts: list[dict] = []
        self._last_used: dict[str, float] = {}  # email -> timestamp
    ...
    def _load(self) -> None:
        ...
        try:
            data = json.loads(self.path.read_text())
            self._accounts = data if isinstance(data, list) else data.get("accounts", [])
        except (ValueError, OSError):
            self._accounts = []

    def reload(self) -> None:
        """Re-read accounts from disk (call after editing accounts.json)."""
        with self._lock:
            self._load()
    ...
    def pick(self) -> Optional[dict]:
        """Pick an account that isn't on cooldown. Returns None if all are cooling down."""
        import time as _time
        now = _time.monotonic()
        with self._lock:
            # Find accounts not on cooldown
            available = [
                acc for acc in self._accounts
                if self._last_used.get(acc["email"], 0) + self.cooldown <= now
            ]
            if not available:
                return None
            acc = random.choice(available)
            self._last_used[acc["email"]] = now
            return acc

    def mark_used(self, email: str) -> None:
        import time as _time
        with self._lock:
            self._last_used[email] = _time.monotonic()
